...
class CounterExampleFinder():
    ...
    def find_init(self, B, sample_nums=10):
        x = sp.symbols(['x{}'.format(i + 1) for i in range(self.ex.n1)])
        samples = []
        satisfy = True
        I_zones = self.I_zones
        ...
        for zone in I_zones:
            ...
            x0 = np.zeros(shape=self.ex.n1)
            cons = []
            for lam in zone:
                cons.append(NonlinearConstraint(lam, 0, np.inf))

            res = minimize(fun=lambda x: -opt(*x), x0=x0, constraints=cons)

            if -res.fun < 0:
                pass
            else:
                result = res.x[:self.ex.h_n]
                print('Counterexamples found in the initial set:', result)
                samples.append(result)
                samples.extend(self.generate_sample(result, sample_nums - 1))
                satisfy = False
    ...
    def find_unsafe(self, B, sample_nums=10):
        x = sp.symbols(['x{}'.format(i + 1) for i in range(self.ex.n2)])
        samples = []
        satisfy = True
        U_zones = self.U_zones
        if isinstance(B, str):
            B = sp.sympify(B)
        opt = sp.lambdify(x, B)
    ...
    def generate_sample(self, center, sample_nums=10):
        ...
        for i in range(sample_nums):
            rd = (np.random.random(self.ex.h_n) - 0.5) * self.eps
            rd = rd + center
            result.append(rd)
        return result

    def is_counter_example(self, B, x, condition: str) -> bool:
        if condition not in ['init', 'unsafe']:
            raise ValueError(f'{condition} is not in validation condition!')

        d = {'x{}'.format(i + 1): x[i] for i in range(self.ex.h_n)}
        b_numerical = B.subs(d)
        # dot_b = sum([sp.diff(B, self.x[i]) * self.f[i](x) for i in range(self.n)])
        # dot_b_numerical = dot_b.subs(d)
        if condition == 'init' and b_numerical > 0 and self.is_domain(x):
            return True
        if condition == 'unsafe' and b_numerical < 0 and self.is_domain(x):
            return True
        # if condition == 'diff' and dot_b_numerical > 0:
        #     return True
        return False

    def is_domain(self, x):
        state = True
        for i in range(self.ex.h_n):
            state = state and (x[i] >= self.ex.D_zones[i][0]) and (x[i] <= self.ex.D_zones[i][1])
        return state

    def get_counter_example(self, B):
        samples = []
        S, satisfy1 = self.find_init(B, 1000)
        samples.append(torch.Tensor(S))
        S, satisfy2 = self.find_unsafe(B, 1000)
        samples.append(torch.Tensor(S))
        S, satisfy3 = [], True
