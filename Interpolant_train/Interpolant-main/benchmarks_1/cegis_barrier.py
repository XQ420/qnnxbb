...
from benchmarks_1.CounterExampleFind_B import CounterExampleFinder
from benchmarks_1.SosVerify_B import SosValidator_B
from benchmarks_1.net_B import Learner
...
class Cegis:
    ...
    def generate_data(self):
        if self.ex.name == 'cav20_2':
            S_i = np.load(f'../data/{self.ex.name}_I.npy')
            S_i = S_i[:8000, :]
            S_u = np.load(f'../data/{self.ex.name}_U.npy')
        else:
            ...
            S_d = np.random.rand(N, self.ex.n1)
            ...
            vis_I = [False for _ in range(N)]
            # print(S_d)
            for i, zone in enumerate(self.I_zones):
                ...
                if self.ex.I_eq is not None:
                    for lam in self.ex.I_eq[i]:
                        cur2 = list(map(lam, S_d.tolist()))
                        cur2 = [(-margin <= e <= margin) for e in cur2]
                        cur1 = [(x and y) for x, y in zip(cur1, cur2)]
            ...
            S_d = np.random.rand(N, self.ex.n2)
            S_d = S_d * D_len + self.D_zones[0]
            vis_U = [False for _ in range(N)]

            for i, zone in enumerate(self.U_zones):
                cur1 = [True for _ in range(N)]
                for lam in zone:
                    cur2 = list(map(lam, S_d.tolist()))
                    cur2 = [e >= 0 for e in cur2]
            ...
            S_i = S_i[:, :self.ex.h_n]
            S_u = S_u[:, :self.ex.h_n]
            np.save(f'../data/{self.ex.name}_I.npy', S_i)
            np.save(f'../data/{self.ex.name}_U.npy', S_u)

        # if self.ex.name == 'cav20_4_1':
        #     D_len = self.D_zones[1] - self.D_zones[0]
        #     N = 3 * self.batch_size  # 初始采样点
        #     S_d = np.random.rand(N, self.ex.h_n)
        #     S_d = S_d * D_len + self.D_zones[0]
        #
